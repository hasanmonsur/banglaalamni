  ...
  function fun_member_save($member_info) {
    ...
    if($membertype!='') {     
      $max_mem_id=fun_last_memberid($membertype);
       ...
       $mimid=rand(1, 99);
       $max_mem_id=date("Ymdhis", strtotime("now")).str_pad($mimid,2,'0',STR_PAD_LEFT);
       $memberid='T'.$max_mem_id;
       $rmpaystatus='N';    
    }
    ...
    if($savestat){
      //echo json_encode("Dear member your registration success..Member ID: ".$memberid);
