...
$convid=$_GET['convid'];
$tranid=$_GET['tranid'];
$mobileid=$_GET['mobile'];
//session_destroy();


if($memberid!="" || $mobileid!="" ){ 

 $traninfo=fun_data_search($memberid,$convid,$tranid,$mobileid); 
...
function fun_data_search($memberid,$convid,$tranid,$mobileid) {
    ...
    " from  tbl_transaction where mem_id='".$memberid."' and con_id='".$convid."' AND (tran_id='".$dbtranid."' OR tran_id =(select b.tran_id from  tbl_payinfo b where chl_trxid='".$bktranid."'))";
    ...
    $query2 = "select reg_id,mem_id,con_id,p_type, reg_name,reg_gender,reg_dress,reg_dress_type,reg_amount,tran_id,reg_status,entry_date ".
              " from tbl_registration where reg_status='Y' AND mem_id='".$memberid."' and con_id='".$convid."' and ('".$dbtranid."'='' OR tran_id='".$dbtranid."') order by reg_id asc";
    //var_dump($query2);
    ...
    while ($row1 = mysqli_fetch_assoc($result2)) {        
        ...
        $regi=new guest($mem_id,$rcount+1,$reg_name." (".$mem_id." )", $reg_gender, $reg_dress,$reg_dress_type,$reg_amount);
...
{
    $query3 = "select mem_id,mem_name ".
    " from  tbl_member where ('".$memberid."'='' OR mem_id='".$memberid."') and ('".$mobileid."'='' OR mem_contact='".$mobileid."')";
    mysqli_set_charset($mylink, 'utf8'); 
    //var_dump($query3);
    $result3 = mysqli_query($mylink,$query3);
    while ($row = mysqli_fetch_assoc($result3)) {
        $mem_id=$row['mem_id']."- ".$row['mem_name'];
        $con_id=$convid;          
    }


    if($mem_id!='')
    $traninfo=new tran("",$mem_id, $con_id, "","","",null,null);
    else 
    $traninfo=new tran("","","","","","",null,null);    
